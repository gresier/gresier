...
function GetPublicServers(a)
	...
	while c do
		...
		local e = Http:JSONDecode(game:HttpGet(d))
...
addcmd('serverhop', {'shop'}, function(args, speaker)
	local Cache = Mem:HasItem('JobId_CACHE_IY') and Http:JSONDecode(Mem:GetItem('JobId_CACHE_IY')) or {}
	if not table.find(Cache, game.JobId) then
		Cache[#Cache + 1] = game.JobId
	end
	Mem:RemoveItem('JobId_CACHE_IY')
	Mem:SetItem('JobId_CACHE_IY', Http:JSONEncode(Cache))
	local Servers = {}
	for _, v in ipairs(GetPublicServers(game.PlaceId)) do
	...
	if #Servers > 0 then
		...
		TP:TeleportToPlaceInstance(game.PlaceId, Servers[1].id, speaker)
	else
		Mem:RemoveItem('JobId_CACHE_IY')
	...
	for i = 1, LOOP_NUM do
		...
		while #t > 0 do
			for _, v in ipairs(t) do
				...
				if not table.find(Tools, v.Handle) then
		...
		if i == LOOP_NUM or i % 5 == 0 then
			...
			if type(firetouchinterest) == "function" and (function()
				if not identifyexecutor then
					return true
				end
				return identifyexecutor() ~= "ScriptWare"
			end)() then
		...
		TempPos = TempPos + Vector3.new(3, 0, 0)
	end
